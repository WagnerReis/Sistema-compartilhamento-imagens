...
let mainUser = {name: "Wagner Reis", email: "wagner@email.com", password: "123456"};
...
describe("Autenticação",() => {
  ...
  test("Deve impedir que um usuário não cadastrado se logue", () => {
    return request.post("/auth")
      .send({email: "email@email.com", password: "abc123"})
      .then(res => {
  ...
  test("Deve impedir que um usuário se logue com uma senha errada", () => {
    return request.post("/auth")
      .send({email: mainUser.email, password: "abc123"})
      .then(res => {
        expect(res.statusCode).toEqual(403);
